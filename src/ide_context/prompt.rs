...
//!
//! Vendored from `codex-rs/tui/src/ide_context/prompt.rs`. Codex folds the
//! rendered context into a `Vec<UserInput>`; our submit pipeline hands a single
//! `String` to the agent loop, so `apply_ide_context_to_user_input` is replaced
//! by `apply_ide_context_to_text` while the rendering stays byte-for-byte the
//! same (including the desktop/extension delimiter, so transcripts stay
//! cross-surface compatible).
...
/// Render the IDE context prefix in front of `text`, returning `Some` when there
/// was any context to inject and `None` otherwise.
pub(crate) fn apply_ide_context_to_text(context: &IdeContext, text: &str) -> Option<String> {
    let context_text = render_prompt_context(context)?;
...
/// Split a user message at the last request delimiter, returning the trimmed
/// request text and its byte offset. Used to strip IDE context back out of a
/// stored transcript.
#[allow(dead_code)]
...
mod tests {
    ...
    fn apply_ide_context_uses_desktop_prompt_request_delimiter() {
        ...
        let prefixed =
            apply_ide_context_to_text(&context, "Ask $figma").expect("prefixed user text");
