...
use crate::sandbox::protocol::NetworkSandboxPolicy;
...
pub fn seatbelt_command_argv(
    ...
    let params = crate::sandbox::sandboxing::seatbelt::CreateSeatbeltCommandArgsParams {
        command,
    ...
    let tail = crate::sandbox::sandboxing::seatbelt::create_seatbelt_command_args(params).ok()?;
    let mut argv = Vec::with_capacity(tail.len() + 1);
    argv.push(crate::sandbox::sandboxing::seatbelt::MACOS_PATH_TO_SEATBELT_EXECUTABLE.to_string());
    argv.extend(tail);
    Some(argv)
}
