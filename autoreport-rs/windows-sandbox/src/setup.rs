...
const SETUP_EXE_FILENAME: &str = "autoreport-windows-sandbox-setup.exe";
...
pub fn sandbox_dir(autoreport_home: &Path) -> PathBuf {
    autoreport_home.join(".sandbox")
}

pub fn sandbox_bin_dir(autoreport_home: &Path) -> PathBuf {
    autoreport_home.join(".sandbox-bin")
}

pub fn sandbox_secrets_dir(autoreport_home: &Path) -> PathBuf {
    autoreport_home.join(".sandbox-secrets")
}

pub fn setup_marker_path(autoreport_home: &Path) -> PathBuf {
    sandbox_dir(autoreport_home).join("setup_marker.json")
}

pub fn sandbox_users_path(autoreport_home: &Path) -> PathBuf {
    sandbox_secrets_dir(autoreport_home).join("sandbox_users.json")
}
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    let mut read_roots = gather_read_roots(command_cwd, &permissions, env_map, autoreport_home);
...
fn run_setup_refresh_inner(
    ...
    let payload = ElevationPayload {
        ...
        autoreport_home: request.autoreport_home.to_path_buf(),
    ...
    let sbx_dir = sandbox_dir(request.autoreport_home);
    let log_path = current_log_file_path(&sbx_dir);
    let cleared_report = match clear_setup_error_report(request.autoreport_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
    ...
    let cwd = std::env::current_dir().unwrap_or_else(|_| request.autoreport_home.to_path_buf());
    log_note(
        &format!(
    ...
    if let Err(err) = clear_setup_error_report(request.autoreport_home) {
        log_note(
...
fn gather_helper_read_roots(autoreport_home: &Path) -> Vec<PathBuf> {
    let helper_dir = helper_bin_dir(autoreport_home);
...
fn gather_full_read_roots_for_permissions(
    ...
    let mut roots = gather_helper_read_roots(autoreport_home);
    roots.extend(
...
pub(crate) fn effective_write_roots_for_permissions(
    ...
    filter_sensitive_write_roots(write_roots, autoreport_home)
}
...
struct ElevationPayload {
    ...
    otel: Option<autoreport_otel::StatsigMetricsSettings>,
    real_user: String,
    mode: SetupMode,
    #[serde(default)]
    refresh_only: bool,
...
fn report_helper_failure(
    ...
    match read_setup_error_report(autoreport_home) {
...
fn verify_setup_completed(autoreport_home: &Path) -> Result<()> {
    if sandbox_setup_is_complete(autoreport_home) {
        Ok(())
    } else {
        Err(failure(
...
fn run_setup_exe(
    ...
    let cleared_report = match clear_setup_error_report(autoreport_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
                &format!(
                ...
                Some(&sandbox_dir(autoreport_home)),
            );
            false
        }
    };
    ...
    if !needs_elevation {
        ...
        verify_setup_completed(autoreport_home)?;
        if let Err(err) = clear_setup_error_report(autoreport_home) {
            log_note(
                &format!(
...
fn run_elevated_setup_inner(
    ...
    let payload = ElevationPayload {
        ...
        allow_local_binding: offline_proxy_settings.allow_local_binding,
        real_user: std::env::var("USERNAME").unwrap_or_else(|_| "Administrators".to_string()),
        otel: autoreport_otel::global_statsig_metrics_settings(),
        mode: SetupMode::Full,
        refresh_only: false,
    };
    ...
    run_setup_exe(&payload, needs_elevation, request.autoreport_home)
}

pub fn run_elevated_provisioning_setup(autoreport_home: &Path, real_user: &str) -> Result<()> {
    let sbx_dir = sandbox_dir(autoreport_home);
    std::fs::create_dir_all(&sbx_dir).map_err(|err| {
        failure(
            SetupErrorCode::OrchestratorSandboxDirCreateFailed,
            format!("failed to create sandbox dir {}: {err}", sbx_dir.display()),
    ...
    let payload = ElevationPayload {
        ...
        command_cwd: autoreport_home.to_path_buf(),
        read_roots: Vec::new(),
        write_roots: Vec::new(),
    ...
    run_setup_exe(&payload, /*needs_elevation*/ false, autoreport_home)
}
...
fn build_payload_roots(
    ...
    let mut read_roots = if let Some(roots) = overrides.read_roots.as_deref() {
        ...
        let mut read_roots = gather_helper_read_roots(request.autoreport_home);
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, autoreport_home: &Path) -> Vec<PathBuf> {
    ...
    let sbx_bin_dir_key = canonical_path_key(&sandbox_bin_dir(autoreport_home));
    let sbx_bin_dir_prefix = format!("{}/", sbx_bin_dir_key.trim_end_matches('/'));
    let secrets_dir_key = canonical_path_key(&sandbox_secrets_dir(autoreport_home));
    let secrets_dir_prefix = format!("{}/", secrets_dir_key.trim_end_matches('/'));

    roots.retain(|root| {
        let key = canonical_path_key(root);
        key != autoreport_home_key
            && key != sbx_dir_key
...
mod tests {
    ...
    fn setup_completion_requires_ready_artifacts() {
        ...
        let err = verify_setup_completed(autoreport_home.path())
    ...
    fn setup_exe_lookup_checks_package_resource_dir_for_bin_exe() {
        ...
        let setup_exe = resources_dir.join("autoreport-windows-sandbox-setup.exe");
    ...
    fn expanded_write_roots_still_drop_protected_autoreport_home() {
        ...
        let roots = super::filter_sensitive_write_roots(roots, &autoreport_home);
    ...
    fn gather_read_roots_includes_helper_bin_dir() {
        ...
        let expected =
            dunce::canonicalize(helper_bin_dir(&autoreport_home)).expect("canonical helper dir");
    ...
    fn build_payload_roots_preserves_helper_roots_when_read_override_is_provided() {
        ...
        let (read_roots, write_roots) = build_payload_roots(
            &super::SandboxSetupRequest {
                ...
                command_cwd: &command_cwd,
                env_map: &HashMap::new(),
                autoreport_home: &autoreport_home,
                proxy_enforced: false,
            },
    ...
    fn effective_write_roots_match_payload_filtering_for_overrides() {
        ...
        let sandbox_root = super::sandbox_dir(&autoreport_home);
        fs::create_dir_all(&autoreport_home).expect("create autoreport home");
        fs::create_dir_all(&command_cwd).expect("create workspace");
        ...
        let forbidden_autoreport_home =
            dunce::canonicalize(&autoreport_home).expect("canonical autoreport home");
        ...
        assert!(!effective_write_roots.contains(&forbidden_autoreport_home));
