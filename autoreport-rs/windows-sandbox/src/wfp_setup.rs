...
use anyhow::Result;
use autoreport_otel::OtelExporter;
use autoreport_otel::OtelProvider;
use autoreport_otel::OtelSettings;
use autoreport_otel::StatsigMetricsSettings;
use std::collections::BTreeMap;
use std::path::Path;

const WFP_SETUP_SERVICE_NAME: &str = "autoreport-windows-sandbox-setup";
const WFP_SETUP_SUCCESS_METRIC: &str = "autoreport.windows_sandbox.wfp_setup_success";
const WFP_SETUP_FAILURE_METRIC: &str = "autoreport.windows_sandbox.wfp_setup_failure";
...
fn build_wfp_metrics_provider(
    ...
    // The setup helper cannot call autoreport-core's OTEL builder because core
...
fn emit_wfp_setup_metric(
    ...
    let Some(provider) = build_wfp_metrics_provider(autoreport_home, otel)? else {
        return Ok(());
    };
    ...
    F: FnMut(&str),
{
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        emit_wfp_setup_metric(autoreport_home, otel, metric)
    }));
    match result {
        Ok(Ok(())) => {}
    ...
    F: FnMut(&str),
{
    ...
    emit_wfp_setup_metric_safely(autoreport_home, otel, offline_username, &metric, &mut log);
}
