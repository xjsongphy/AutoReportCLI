...
mod tests {
    ...
    #[test]
    fn find_autoreport_home_env_missing_path_is_fatal() {
    ...
    #[test]
    fn find_autoreport_home_env_file_path_is_fatal() {
        let temp_home = TempDir::new().expect("temp home");
        let file_path = temp_home.path().join("codex-home.txt");
        fs::write(&file_path, "not a directory").expect("write temp file");
        let file_str = file_path
            .to_str()
            .expect("file autoreport home path should be valid utf-8");

        let err = find_autoreport_home_from_env(Some(file_str)).expect_err("file AUTOREPORT_HOME");
    ...
    #[test]
    fn find_autoreport_home_env_valid_directory_canonicalizes() {
        let temp_home = TempDir::new().expect("temp home");
