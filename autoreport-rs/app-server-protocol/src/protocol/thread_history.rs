...
use autoreport_codex_protocol::protocol::AgentReasoningRawContentEvent;
use autoreport_codex_protocol::protocol::AgentStatus;
...
use autoreport_codex_protocol::protocol::ContextCompactedEvent;
use autoreport_codex_protocol::protocol::DynamicToolCallResponseEvent;
use autoreport_codex_protocol::protocol::ErrorEvent;
...
use autoreport_codex_protocol::protocol::GuardianAssessmentEvent;
use autoreport_codex_protocol::protocol::GuardianAssessmentStatus;
use autoreport_codex_protocol::protocol::ImageGenerationBeginEvent;
use autoreport_codex_protocol::protocol::ImageGenerationEndEvent;
use autoreport_codex_protocol::protocol::ItemCompletedEvent;
use autoreport_codex_protocol::protocol::ItemStartedEvent;
use autoreport_codex_protocol::protocol::McpToolCallBeginEvent;
use autoreport_codex_protocol::protocol::McpToolCallEndEvent;
use autoreport_codex_protocol::protocol::PatchApplyBeginEvent;
use autoreport_codex_protocol::protocol::PatchApplyEndEvent;
use autoreport_codex_protocol::protocol::RolloutItem;
use autoreport_codex_protocol::protocol::ThreadRolledBackEvent;
use autoreport_codex_protocol::protocol::TurnAbortedEvent;
use autoreport_codex_protocol::protocol::TurnCompleteEvent;
use autoreport_codex_protocol::protocol::TurnStartedEvent;
use autoreport_codex_protocol::protocol::UserMessageEvent;
use autoreport_codex_protocol::protocol::ViewImageToolCallEvent;
...
#[cfg(test)]
use autoreport_codex_protocol::protocol::ExecCommandStatus as CoreExecCommandStatus;
#[cfg(test)]
...
impl ThreadHistoryBuilder {
    ...
    fn handle_response_item(&mut self, item: &autoreport_codex_protocol::models::ResponseItem) {
        let autoreport_codex_protocol::models::ResponseItem::Message {
            role, content, id, ..
        } = item
        else {
            return;
    ...
    fn handle_materialized_item_lifecycle(
        &mut self,
        turn_id: &str,
        item: &autoreport_codex_protocol::items::TurnItem,
    ) {
        let is_review_mode_item = matches!(
            item,
            autoreport_codex_protocol::items::TurnItem::EnteredReviewMode(_)
                | autoreport_codex_protocol::items::TurnItem::ExitedReviewMode(_)
        );
        let should_upsert = match item {
            autoreport_codex_protocol::items::TurnItem::Plan(plan) => !plan.text.is_empty(),
            autoreport_codex_protocol::items::TurnItem::HookPrompt(_)
            | autoreport_codex_protocol::items::TurnItem::CommandExecution(_)
            | autoreport_codex_protocol::items::TurnItem::DynamicToolCall(_)
            | autoreport_codex_protocol::items::TurnItem::CollabAgentToolCall(_)
            | autoreport_codex_protocol::items::TurnItem::SubAgentActivity(_)
            | autoreport_codex_protocol::items::TurnItem::Extension(_)
            | autoreport_codex_protocol::items::TurnItem::EnteredReviewMode(_)
            | autoreport_codex_protocol::items::TurnItem::ExitedReviewMode(_) => true,
            autoreport_codex_protocol::items::TurnItem::UserMessage(_)
            | autoreport_codex_protocol::items::TurnItem::AgentMessage(_)
            | autoreport_codex_protocol::items::TurnItem::Reasoning(_)
            | autoreport_codex_protocol::items::TurnItem::WebSearch(_)
            | autoreport_codex_protocol::items::TurnItem::ImageView(_)
            | autoreport_codex_protocol::items::TurnItem::ImageGeneration(_)
            | autoreport_codex_protocol::items::TurnItem::FileChange(_)
            | autoreport_codex_protocol::items::TurnItem::McpToolCall(_)
            | autoreport_codex_protocol::items::TurnItem::ContextCompaction(_) => false,
        };
    ...
    fn handle_dynamic_tool_call_request(
        &mut self,
        payload: &autoreport_codex_protocol::dynamic_tools::DynamicToolCallRequest,
    ) {
    ...
    fn handle_collab_agent_spawn_begin(
        &mut self,
        payload: &autoreport_codex_protocol::protocol::CollabAgentSpawnBeginEvent,
    ) {
    ...
    fn handle_collab_agent_spawn_end(
        &mut self,
        payload: &autoreport_codex_protocol::protocol::CollabAgentSpawnEndEvent,
    ) {
    ...
    fn handle_collab_agent_interaction_begin(
        &mut self,
        payload: &autoreport_codex_protocol::protocol::CollabAgentInteractionBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_agent_interaction_end(
        &mut self,
        payload: &autoreport_codex_protocol::protocol::CollabAgentInteractionEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
    ...
    fn handle_sub_agent_activity(
        &mut self,
        payload: &autoreport_codex_protocol::protocol::SubAgentActivityEvent,
    ) {
    ...
    fn handle_collab_waiting_begin(
        &mut self,
        payload: &autoreport_codex_protocol::protocol::CollabWaitingBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_waiting_end(
        &mut self,
        payload: &autoreport_codex_protocol::protocol::CollabWaitingEndEvent,
    ) {
    ...
    fn handle_collab_close_begin(
        &mut self,
        payload: &autoreport_codex_protocol::protocol::CollabCloseBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_resume_begin(
        &mut self,
        payload: &autoreport_codex_protocol::protocol::CollabResumeBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_resume_end(
        &mut self,
        payload: &autoreport_codex_protocol::protocol::CollabResumeEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
    ...
    fn handle_entered_review_mode(
        &mut self,
        payload: &autoreport_codex_protocol::protocol::EnteredReviewModeEvent,
    ) {
        let review = payload
            .user_facing_hint
            .clone()
    ...
    fn handle_exited_review_mode(
        &mut self,
        payload: &autoreport_codex_protocol::protocol::ExitedReviewModeEvent,
    ) {
...
fn convert_dynamic_tool_content_items(
    items: &[autoreport_codex_protocol::dynamic_tools::DynamicToolCallOutputContentItem],
...
mod tests {
    ...
    use autoreport_codex_protocol::items::CommandExecutionStatus as CoreCommandExecutionStatus;
    use autoreport_codex_protocol::items::EnteredReviewModeItem as CoreEnteredReviewModeItem;
    use autoreport_codex_protocol::items::ExitedReviewModeItem as CoreExitedReviewModeItem;
    use autoreport_codex_protocol::items::HookPromptFragment as CoreHookPromptFragment;
    use autoreport_codex_protocol::items::TurnItem as CoreTurnItem;
    use autoreport_codex_protocol::items::UserMessageItem as CoreUserMessageItem;
    use autoreport_codex_protocol::items::build_hook_prompt_message;
    use autoreport_codex_protocol::mcp::CallToolResult;
    use autoreport_codex_protocol::models::ImageDetail;
    use autoreport_codex_protocol::models::MessagePhase as CoreMessagePhase;
    use autoreport_codex_protocol::models::WebSearchAction as CoreWebSearchAction;
    use autoreport_codex_protocol::parse_command::ParsedCommand;
    use autoreport_codex_protocol::protocol::AgentMessageEvent;
    use autoreport_codex_protocol::protocol::AgentReasoningEvent;
    use autoreport_codex_protocol::protocol::AgentReasoningRawContentEvent;
    use autoreport_codex_protocol::protocol::ApplyPatchApprovalRequestEvent;
    use autoreport_codex_protocol::protocol::CodexErrorInfo;
    use autoreport_codex_protocol::protocol::CompactedItem;
    use autoreport_codex_protocol::protocol::DynamicToolCallResponseEvent;
    use autoreport_codex_protocol::protocol::EnteredReviewModeEvent;
    use autoreport_codex_protocol::protocol::ExecCommandEndEvent;
    use autoreport_codex_protocol::protocol::ExecCommandSource;
    use autoreport_codex_protocol::protocol::ExitedReviewModeEvent;
    use autoreport_codex_protocol::protocol::ItemStartedEvent;
    use autoreport_codex_protocol::protocol::McpInvocation;
    use autoreport_codex_protocol::protocol::McpToolCallEndEvent;
    use autoreport_codex_protocol::protocol::PatchApplyBeginEvent;
    use autoreport_codex_protocol::protocol::ReviewTarget;
    use autoreport_codex_protocol::protocol::ThreadRolledBackEvent;
    use autoreport_codex_protocol::protocol::TurnAbortReason;
    use autoreport_codex_protocol::protocol::TurnAbortedEvent;
    use autoreport_codex_protocol::protocol::TurnCompleteEvent;
    use autoreport_codex_protocol::protocol::TurnStartedEvent;
    use autoreport_codex_protocol::protocol::UserMessageEvent;
        ...
        let events = vec![
            ...
            EventMsg::ItemStarted(ItemStartedEvent {
                ...
                item: CoreTurnItem::UserMessage(CoreUserMessageItem {
                    ...
                    content: vec![autoreport_codex_protocol::user_input::UserInput::Text {
                        text: "hello".into(),
                        text_elements: Vec::new(),
                    }],
                }),
    ...
    fn reconstructs_dynamic_tool_items_from_request_and_response_events() {
        let events = vec![
            ...
            EventMsg::DynamicToolCallRequest(
                autoreport_codex_protocol::dynamic_tools::DynamicToolCallRequest {
                    call_id: "dyn-1".into(),
                    turn_id: "turn-1".into(),
                    started_at_ms: 0,
        ...
        let events = vec![
            ...
            EventMsg::PatchApplyEnd(PatchApplyEndEvent {
                ...
                changes: [(
                    ...
                    autoreport_codex_protocol::protocol::FileChange::Add {
                        content: "hello\n".into(),
                    },
                )]
                .into_iter()
    ...
    fn reconstructs_collab_resume_end_item() {
        let events = vec![
            ...
            EventMsg::CollabResumeEnd(autoreport_codex_protocol::protocol::CollabResumeEndEvent {
    ...
    fn reconstructs_interrupted_send_input_as_completed_collab_call() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentInteractionBegin(
                autoreport_codex_protocol::protocol::CollabAgentInteractionBeginEvent {
                    call_id: "send-1".into(),
                    started_at_ms: 0,
            ...
            EventMsg::CollabAgentInteractionEnd(
                autoreport_codex_protocol::protocol::CollabAgentInteractionEndEvent {
                    call_id: "send-1".into(),
                    completed_at_ms: 0,
                    sender_thread_id: sender,
                    receiver_thread_id: receiver,
