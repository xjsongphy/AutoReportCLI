...
fn expected_sha256() -> Option<[u8; 32]> {
    ...
    *EXPECTED.get_or_init(|| {
        let raw_digest = option_env!("AUTOREPORT_BWRAP_SHA256")?;
        let digest = parse_sha256_hex(raw_digest)
            .unwrap_or_else(|err| panic!("invalid AUTOREPORT_BWRAP_SHA256 value: {err}"));
...
mod tests {
    ...
    fn finds_package_layout_bwrap_from_install_context() {
        ...
        let resources_dir = package_dir.join("autoreport-resources");
    ...
    fn finds_legacy_standalone_bundled_bwrap_next_to_exe_resources() {
        ...
        let expected_bwrap = temp_dir.path().join("autoreport-resources").join("bwrap");
    ...
    fn finds_npm_bundled_bwrap_next_to_target_vendor_dir() {
        ...
        let expected_bwrap = target_dir.join("autoreport-resources").join("bwrap");
        write_executable(&exe);
        write_executable(&expected_bwrap);
