...
use autoreport_network_proxy::write_attribution_frame;
...
const PROXY_SOCKET_DIR_PREFIX: &str = "autoreport-linux-sandbox-proxy-";
...
fn proxy_socket_parent_dir() -> PathBuf {
    if let Some(autoreport_home) = std::env::var_os("AUTOREPORT_HOME") {
        let candidate = PathBuf::from(autoreport_home).join("tmp");
...
fn harden_bridge_process() -> io::Result<()> {
    ...
    autoreport_process_hardening::disable_process_dumping()
}
...
mod tests {
    ...
    fn cleanup_proxy_socket_dir_removes_bridge_artifacts() {
        ...
        let socket_dir = root.path().join("autoreport-linux-sandbox-proxy-test");
    ...
    fn parse_proxy_socket_dir_owner_pid_reads_owner_pid() {
        assert_eq!(
            parse_proxy_socket_dir_owner_pid("autoreport-linux-sandbox-proxy-1234-0"),
            Some(1234)
        );
        assert_eq!(
            parse_proxy_socket_dir_owner_pid("autoreport-linux-sandbox-proxy-1234-1000-0"),
            Some(1234)
        );
        assert_eq!(
            parse_proxy_socket_dir_owner_pid("autoreport-linux-sandbox-proxy-x"),
            None
        );
