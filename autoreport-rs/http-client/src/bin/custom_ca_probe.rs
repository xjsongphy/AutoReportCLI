...
fn build_probe_client(
    ...
    if let Some(proxy_url) = proxy_url {
        ...
        return autoreport_http_client::build_reqwest_client_with_custom_ca(builder.proxy(proxy))
            .map_err(|error| error.to_string());
    }

    autoreport_http_client::build_reqwest_client_for_subprocess_tests(builder)
        .map_err(|error| error.to_string())
}
