...
use autoreport_codex_rollout::StateDbHandle;
...
impl LocalThreadStoreConfig {
    pub fn from_config(config: &impl autoreport_codex_rollout::RolloutConfigView) -> Self {
        Self {
    ...
    async fn thread_history_db(&self) -> ThreadStoreResult<&sqlx::SqlitePool> {
        self.thread_history_db
            .get_or_try_init(|| async {
                autoreport_state::open_thread_history_db(&self.config.sqlite).await
            })
            .await
...
mod tests {
    ...
    use autoreport_codex_protocol::models::FunctionCallOutputPayload;
        ...
        live_thread
            .append_items(&[
                ...
                RolloutItem::EventMsg(EventMsg::TokenCount(
                    autoreport_codex_protocol::protocol::TokenCountEvent {
                        info: None,
                        rate_limits: None,
                    },
                )),
