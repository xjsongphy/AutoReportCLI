...
use autoreport_codex_protocol::models::AgentMessageInputContent;
...
async fn fork_context_excludes_items_after_frozen_cutoff() {
    ...
    let session_meta = autoreport_codex_rollout::read_session_meta_line(path.as_path())
        .await
...
async fn replays_nested_archived_lineage_from_frozen_prefix() {
    ...
    let child_meta = autoreport_codex_rollout::read_session_meta_line(child_path.as_path())
        .await
...
async fn assert_reverse_scan_matches_full_history(home: &Path, path: &Path) {
    let session_meta = autoreport_codex_rollout::read_session_meta_line(path)
        .await
