...
use autoreport_codex_rollout::append_rollout_item_to_path;
use autoreport_codex_rollout::append_thread_name;
...
pub(super) async fn update_thread_metadata(
    ...
    autoreport_codex_rollout::state_db::reconcile_rollout(
        ...
        async {
            ...
            if existing.is_none()
                ...
                && let Some(existing_rollout_path) =
                    autoreport_codex_rollout::existing_rollout_path(path).await
            {
                autoreport_codex_rollout::state_db::reconcile_rollout(
...
async fn metadata_for_missing_sqlite_row(
    ...
    patch: &ThreadMetadataPatch,
) -> ThreadStoreResult<autoreport_state::ThreadMetadata> {
...
async fn canonical_history_mode(
    ...
    let session_meta = match read_session_meta_line(rollout_path).await {
        ...
        Err(err) => {
            if autoreport_codex_rollout::existing_rollout_path(rollout_path)
                .await
                .is_none()
...
fn normalize_cwd(cwd: PathBuf) -> PathBuf {
    autoreport_utils_path::normalize_for_path_comparison(cwd.as_path()).unwrap_or(cwd)
}

async fn apply_thread_git_info_patch(
    state_db: &autoreport_state::StateRuntime,
    thread_id: ThreadId,
...
async fn apply_thread_git_info_to_rollout(
    ...
    session_meta.git = Some(GitInfo {
        commit_hash: sha.as_deref().map(autoreport_git_utils::GitSha::new),
        branch: branch.clone(),
...
mod tests {
    ...
    async fn update_thread_metadata_sets_name_on_active_rollout_and_indexes_name() {
        ...
        let latest_name = autoreport_codex_rollout::find_thread_name_by_id(home.path(), &thread_id)
            .await
    ...
    async fn paginated_name_updates_use_sqlite_without_rollout_writes() {
        ...
        assert_eq!(
            autoreport_codex_rollout::find_thread_name_by_id(home.path(), &thread_id)
                .await
                .expect("find thread name")
                .as_deref(),
