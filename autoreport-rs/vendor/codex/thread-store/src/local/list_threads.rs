...
pub(super) async fn list_threads(
    ...
    let sort_key = match params.sort_key {
        ThreadSortKey::CreatedAt => autoreport_codex_rollout::ThreadSortKey::CreatedAt,
        ThreadSortKey::UpdatedAt => autoreport_codex_rollout::ThreadSortKey::UpdatedAt,
        ThreadSortKey::RecencyAt => autoreport_codex_rollout::ThreadSortKey::RecencyAt,
    };
    let sort_direction = match params.sort_direction {
        SortDirection::Asc => autoreport_codex_rollout::SortDirection::Asc,
        SortDirection::Desc => autoreport_codex_rollout::SortDirection::Desc,
    };
...
pub(super) async fn list_rollout_threads(
    state_db: Option<autoreport_codex_rollout::StateDbHandle>,
    config: &RolloutConfig,
    default_model_provider_id: &str,
    params: &ListThreadsParams,
    cursor: Option<&autoreport_codex_rollout::Cursor>,
    sort_key: autoreport_codex_rollout::ThreadSortKey,
    sort_direction: autoreport_codex_rollout::SortDirection,
) -> ThreadStoreResult<autoreport_codex_rollout::ThreadsPage> {
        ...
        let relation_filter = params
            ...
            .map(|relation_filter| match relation_filter {
                ThreadRelationFilter::DirectChildrenOf(parent_thread_id) => {
                    autoreport_state::ThreadRelationFilter::DirectChildrenOf(parent_thread_id)
                }
                ThreadRelationFilter::DescendantsOf(ancestor_thread_id) => {
                    autoreport_state::ThreadRelationFilter::DescendantsOf(ancestor_thread_id)
                }
            });
        let page = autoreport_codex_rollout::state_db::list_threads_db(
            state_db.as_deref(),
            &config.sqlite,
            params.page_size,
            cursor,
...
mod tests {
    ...
    async fn list_threads_preserves_sqlite_title_search_results() {
        ...
        let runtime = autoreport_state::StateRuntime::init(
        ...
        let mut builder = autoreport_state::ThreadMetadataBuilder::new(
            thread_id,
            rollout_path,
            created_at,
            SessionSource::Cli,
    ...
    async fn list_paginated_threads_uses_sqlite_name_over_legacy_compatibility() {
        ...
        runtime
            ...
            .await
            .expect("state db upsert should succeed");
        autoreport_codex_rollout::append_thread_name(home.path(), thread_id, "stale index name")
            .await
