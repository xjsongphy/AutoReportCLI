...
use autoreport_codex_rollout::find_archived_thread_path_by_id_str;
use autoreport_codex_rollout::find_thread_name_by_id;
...
async fn resolve_requested_rollout_path(
    ...
    let Some(path) = autoreport_codex_rollout::existing_rollout_path(path.as_path()).await else {
        return Err(ThreadStoreError::InvalidRequest {
            message: format!(
...
pub(super) async fn resolve_rollout_path(
    ...
    if let Ok(path) = live_writer::rollout_path(store, thread_id).await
        && autoreport_codex_rollout::existing_rollout_path(path.as_path())
            .await
            .is_some()
...
async fn read_thread_from_rollout_path(
    ...
    thread.rollout_path = Some(autoreport_codex_rollout::plain_rollout_path(path.as_path()));
...
pub(super) async fn load_history_items(
    path: &std::path::Path,
) -> ThreadStoreResult<Vec<autoreport_codex_protocol::protocol::RolloutItem>> {
...
async fn stored_thread_from_sqlite_metadata(
    ...
    let session_meta = match read_required_session_meta_line(metadata.rollout_path.as_path()).await
    {
        ...
        Err(_)
            if autoreport_codex_rollout::existing_rollout_path(metadata.rollout_path.as_path())
                .await
                .is_none() =>
        {
            None
...
fn stored_thread_from_meta_line(
    ...
    let rollout_path = autoreport_codex_rollout::plain_rollout_path(path.as_path());
    StoredThread {
...
mod tests {
    ...
    async fn read_thread_by_rollout_path_preserves_legacy_name_and_sqlite_git_info() {
        ...
        autoreport_codex_rollout::append_thread_name(home.path(), thread_id, "Latest index name")
            .await
    ...
    async fn read_thread_uses_legacy_thread_name_when_sqlite_title_is_missing() {
        ...
        let thread_id = ThreadId::from_string(&uuid.to_string()).expect("valid thread id");
        write_session_file(home.path(), "2025-01-03T12-00-00", uuid).expect("session file");
        autoreport_codex_rollout::append_thread_name(home.path(), thread_id, "Legacy title")
            .await
            .expect("append legacy thread name");
