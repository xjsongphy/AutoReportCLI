...
struct ThreadSearchItem {
    item: autoreport_codex_rollout::ThreadItem,
    snippet: String,
}
...
pub(super) async fn search_threads(
    ...
    loop {
        ...
        for item in page.items {
            let logical_path = autoreport_codex_rollout::plain_rollout_path(item.path.as_path());
...
) -> Option<autoreport_codex_rollout::Cursor> {
