...
pub fn run_main() -> i32 {
    ...
    let cwd = match autoreport_utils_absolute_path::AbsolutePathBuf::current_dir() {
        Ok(cwd) => cwd,
        Err(err) => {
    ...
    let cwd = autoreport_utils_path_uri::PathUri::from_abs_path(&cwd);
    match runtime.block_on(crate::apply_patch(
        &patch_arg,
        &cwd,
        &mut stdout,
        &mut stderr,
        autoreport_exec_server::LOCAL_FS.as_ref(),
        /*sandbox*/ None,
    )) {
        Ok(_) => {
