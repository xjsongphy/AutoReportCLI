...
pub use autoreport_state::LogEntry;
use autoreport_state::SqliteConfig;
use autoreport_state::ThreadMetadataBuilder;
use autoreport_utils_path::normalize_for_path_comparison;
...
pub type StateDbHandle = Arc<autoreport_state::StateRuntime>;
...
async fn try_init_with_roots_inner(
    ...
    let runtime =
        autoreport_state::StateRuntime::init(sqlite.clone(), default_model_provider_id.clone())
            .await
            .with_context(|| {
                format!(
    ...
    .await;
    autoreport_state::record_backfill_gate(
        /*telemetry*/ None,
...
async fn wait_for_backfill_gate(
    runtime: &autoreport_state::StateRuntime,
    codex_home: &Path,
    ...
    loop {
        ...
        if backfill_state.status == autoreport_state::BackfillStatus::Complete {
            return Ok(());
        }
...
pub async fn get_state_db(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    ...
    let runtime = match autoreport_state::StateRuntime::init(
...
pub fn sqlite_telemetry_recorder(
    ...
    originator: &str,
) -> autoreport_state::DbTelemetryHandle {
...
async fn require_backfill_complete(
    ...
    match runtime.get_backfill_state().await {
        Ok(state) if state.status == autoreport_state::BackfillStatus::Complete => Some(runtime),
        Ok(state) => {
            warn!(
...
fn cursor_to_anchor(cursor: Option<&Cursor>) -> Option<autoreport_state::Anchor> {
    let cursor = cursor?;
...
pub async fn list_thread_ids_db(
    context: Option<&autoreport_state::StateRuntime>,
    sqlite: &autoreport_state::SqliteConfig,
    page_size: usize,
    ...
    match ctx
        .list_thread_ids(
            ...
            anchor.as_ref(),
            match sort_key {
                ThreadSortKey::CreatedAt => autoreport_state::SortKey::CreatedAt,
                ThreadSortKey::UpdatedAt => autoreport_state::SortKey::UpdatedAt,
                ThreadSortKey::RecencyAt => autoreport_state::SortKey::RecencyAt,
            },
...
pub async fn list_threads_db(
    ...
    relation_filter: Option<autoreport_state::ThreadRelationFilter>,
    archived: bool,
    is_pinned: Option<bool>,
    search_term: Option<&str>,
) -> Option<autoreport_state::ThreadsPage> {
    let ctx = context?;
    if ctx.sqlite() != sqlite {
        warn!(
            "state db SQLite home mismatch: expected {}, got {}",
    ...
    let filters = autoreport_state::ThreadFilterOptions {
        ...
        sort_direction: match sort_direction {
            SortDirection::Asc => autoreport_state::SortDirection::Asc,
            SortDirection::Desc => autoreport_state::SortDirection::Desc,
        },
        search_term,
    };
