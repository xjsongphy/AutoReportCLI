...
use autoreport_uds::UnixListener;
use autoreport_uds::UnixStream;
...
pub async fn prepare_control_socket_path(socket_path: &Path) -> IoResult<()> {
    if let Some(parent) = socket_path.parent() {
        autoreport_uds::prepare_private_socket_directory(parent).await?;
    }
    ...
    if !autoreport_uds::is_stale_socket_path(socket_path).await? {
        return Err(std::io::Error::new(
