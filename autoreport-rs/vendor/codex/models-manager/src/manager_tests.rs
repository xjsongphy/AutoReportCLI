...
use autoreport_login::AuthKeyringBackendKind;
use autoreport_login::AuthManager;
...
impl ExternalAuth for TestExternalApiKeyAuth {
    fn resolve(&self) -> autoreport_login::ExternalAuthFuture<'_, CodexAuth> {
    ...
    ) -> autoreport_login::ExternalAuthFuture<'_, CodexAuth> {
        Box::pin(async { Ok(CodexAuth::from_api_key("test-external-api-key")) })
    }
}
...
async fn chatgpt_auth_tokens_for_tests(codex_home: &Path) -> CodexAuth {
    let auth_dot_json = autoreport_login::AuthDotJson {
        auth_mode: Some(AuthMode::ChatgptAuthTokens),
        openai_api_key: None,
        tokens: Some(TokenData {
            id_token: autoreport_login::token_data::parse_chatgpt_jwt_claims(
...
c2ln",
    ...
    CodexAuth::from_auth_storage(
        ...
        &autoreport_login::test_support::transport_default_auth_route_config(),
    )
    .await
