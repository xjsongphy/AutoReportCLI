#!/usr/bin/env bash
# Vendored verbatim from openai/codex (.github/scripts/install-musl-build-tools.sh).
# Sets up a self-contained musl toolchain using `zig cc` (which ships a complete
# musl sysroot with linux UAPI headers, avoiding missing <linux/...> headers),
# builds libcap from source against musl, and exports CC/CFLAGS/PKG_CONFIG vars
# via GITHUB_ENV so the subsequent musl cargo build links cleanly.
